...
func ExampleHandlerOptions_customLevels() {
	...
	logger := slog.New(th)
	logger.Log(context.TODO(), LevelEmergency, "missing pilots")
	logger.Error("failed to start engines", "err", "missing fuel")
	logger.Warn("falling back to default value")
	logger.Log(context.TODO(), LevelNotice, "all systems are running")
	logger.Info("initiating launch")
	logger.Debug("starting background job")
	logger.Log(context.TODO(), LevelTrace, "button clicked")
