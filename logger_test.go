...
func TestLogTextHandler(t *testing.T) {
	...
	l.Log(context.TODO(), LevelWarn+1, "w", Int("a", 1), String("b", "two"))
	check(`level=WARN\+1 msg=w a=1 b=two`)

	l.LogAttrs(context.TODO(), LevelInfo+1, "a b c", Int("a", 1), String("b", "two"))
...
func TestCallDepth(t *testing.T) {
	...
	check := func(count int) {
		t.Helper()
		const wantFunc = "go.innotegrity.dev/slog.TestCallDepth"
	...
	logger.Log(context.TODO(), LevelInfo, "")
	check(0)
	logger.LogAttrs(context.TODO(), LevelInfo, "")
	check(1)
	logger.Debug("")
	check(2)
	logger.Info("")
	...
	Error("")
	check(9)
	Log(context.TODO(), LevelInfo, "")
	check(10)
	LogAttrs(context.TODO(), LevelInfo, "")
	check(11)
}
...
func TestAlloc(t *testing.T) {
	...
	t.Run("logger.Log", func(t *testing.T) {
		wantAllocs(t, 0, func() { dl.Log(context.TODO(), LevelDebug, "hello") })
	})
	...
	t.Run("2 pairs disabled inline", func(t *testing.T) {
		...
		i := 2000
		wantAllocs(t, 2, func() {
			l.Log(context.TODO(), LevelInfo, "hello",
				"n", i,
				"s", s,
			)
		})
	...
	t.Run("2 pairs disabled", func(t *testing.T) {
		...
		i := 2000
		wantAllocs(t, 0, func() {
			if l.Enabled(context.TODO(), LevelInfo) {
				l.Log(context.TODO(), LevelInfo, "hello",
					"n", i,
					"s", s,
				)
	...
	t.Run("attrs1", func(t *testing.T) {
		wantAllocs(t, 0, func() { dl.LogAttrs(context.TODO(), LevelInfo, "", Int("a", 1)) })
		wantAllocs(t, 0, func() { dl.LogAttrs(context.TODO(), LevelInfo, "", Any("error", io.EOF)) })
	})
	t.Run("attrs3", func(t *testing.T) {
		wantAllocs(t, 0, func() {
			dl.LogAttrs(context.TODO(), LevelInfo, "hello", Int("a", 1), String("b", "two"), Duration("c", time.Second))
		})
	})
	t.Run("attrs3 disabled", func(t *testing.T) {
		logger := New(discardHandler{disabled: true})
		wantAllocs(t, 0, func() {
			logger.LogAttrs(context.TODO(), LevelInfo, "hello", Int("a", 1), String("b", "two"), Duration("c", time.Second))
		})
	})
	t.Run("attrs6", func(t *testing.T) {
		wantAllocs(t, 1, func() {
			dl.LogAttrs(context.TODO(), LevelInfo, "hello",
...
func BenchmarkNopLog(b *testing.B) {
	...
	b.Run("no attrs", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			l.LogAttrs(context.TODO(), LevelInfo, "msg")
		}
	})
	b.Run("attrs", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			l.LogAttrs(context.TODO(), LevelInfo, "msg", Int("a", 1), String("b", "two"), Bool("c", true))
		}
	})
	...
	b.Run("keys-values", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			l.Log(context.TODO(), LevelInfo, "msg", "a", 1, "b", "two", "c", true)
		}
	})
